...
<template>
  <div class="menu min-h-screen ">
    ...
    <div
      class="flex flex-col justify-center md:flex-row mt-2 space-y-4 md:space-y-0 md:space-x-8 p-6 md:p-8 w-4/5"
    >
      ...
      >
        ...
        <p class="text-[#8ea6ab] text-base mt-4">
          ...
          <button
            @click="deleteUser"
            class="text-nowrap font-bold text-[#f63939] hover:underline"
          >
            Delete
          </button>
        </p>
        <button @click="logout" class="logout text-white font-bold bg-[#52443b] rounded-md w-28 h-10 mt-4">Logout</button>
      </div>
    ...
    <HowToPlayModal
      ...
      @close="showHowToPlayModal = false"
    />
  </div>
  </div>
</template>
